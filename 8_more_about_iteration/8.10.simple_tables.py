...
in the table the Intel Pentium processor chip used to perform
floating-point division.

Although a power of 2 table is not as useful as it once was, it still makes
a good example of iteration. The following program outputs a sequence of values
in the left column and 2 raised to the power of that value in the right column:
'''

print("n", '\t', "2**n") #table column headings
print("---", '\t', "-----")

for x in range(21): # generate values for columns
    print(x, '\t', 2 ** x)
...
represent invisible characters like tabs and newlines.
...
called the cursor keeps track of where the next character will go.
After a print function is executed, the cursor normally goes to the beginning
of the next line.
...
of the previous program. Because of the tab characters between the columns, the
